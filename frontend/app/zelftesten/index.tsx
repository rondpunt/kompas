...
// Kleur per categorie conform Junie design
const CAT_COLORS: Record<string, string> = {
  alle: BRAND.blue,
  stemming: BRAND.coral,
  angst: BRAND.blue,
  adhd: BRAND.yellow,
  slaap: BRAND.green,
  stress: BRAND.orange,
  relaties: BRAND.coral,
  zelfbeeld: BRAND.orange,
  autisme: BRAND.green,
};
...
export default function ZelftestenIndex() {
  ...
  const renderCard = ({ item }: { item: Assessment }) => {
    const accentColor = CAT_COLORS[item.category] || BRAND.blue;
    return (
      <TouchableOpacity
      ...
      >
        ...
        <View style={styles.cardBody}>
          ...
          <Text style={[styles.cardSubtitle, { color: palette.textSecondary }]} numberOfLines={1}>{item.subtitle}</Text>
          <View style={styles.metaRow}>
            <View style={styles.metaItem}>
              <Feather name="help-circle" size={11} color={palette.textMuted} />
              <Text style={[styles.cardMeta, { color: palette.textMuted }]}>{item.questionCount} vragen</Text>
            </View>
            <View style={[styles.metaDot, { backgroundColor: palette.textFaint }]} />
            <View style={styles.metaItem}>
              <Feather name="clock" size={11} color={palette.textMuted} />
              <Text style={[styles.cardMeta, { color: palette.textMuted }]}>{item.estimatedMinutes} min</Text>
            </View>
          </View>
        </View>
        <View style={[styles.chevWrap, { backgroundColor: accentColor + '18' }]}>
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: palette.background }]}>
      ...
      <View style={styles.headerHero}>
        <Text style={[styles.heroTitle, { color: palette.textPrimary, fontFamily: Platform.select({ ios: "Georgia", android: "serif" }) }]}>
          Wat ben je aan het voelen?
        </Text>
        <Text style={[styles.subtitle, { color: palette.textMuted }]}>24 gevalideerde screeners — indicatie, geen diagnose.</Text>
      </View>
      ...
      <View style={styles.chipsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
          {CATEGORIES.map((c) => {
            ...
            const color = CAT_COLORS[c.id] || BRAND.blue;
            return (
              <TouchableOpacity
                key={c.id}
              ...
              >
                <Text style={[styles.chipText, { color: active ? "#ffffff" : palette.textSecondary }]}>{c.label}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
      ...
      <View style={[styles.footer, { borderTopColor: palette.borderSubtle, backgroundColor: palette.background }]}>
        <View style={styles.footerInner}>
          ...
          <Text style={[styles.footerText, { color: palette.textFaint }]}>Indicatie — geen diagnose</Text>
        </View>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 15, fontWeight: "500", letterSpacing: -0.2 },
  headerHero: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 14 },
  heroTitle: { fontSize: 24, fontWeight: "500", fontStyle: "italic", letterSpacing: -0.4, lineHeight: 30, marginBottom: 6 },
  subtitle: { fontSize: 13, lineHeight: 18 },
  chipsContainer: { paddingBottom: 12 },
  chipsRow: { paddingHorizontal: 16, gap: 7, alignItems: "center" },
  chip: { height: 32, paddingHorizontal: 14, borderRadius: 999, borderWidth: 0.5, alignItems: "center", justifyContent: "center" },
  chipText: { fontSize: 12.5, fontWeight: "500", letterSpacing: 0.1 },
  list: { paddingHorizontal: 16, paddingTop: 4, paddingBottom: 28 },
  card: {
    flexDirection: "row", alignItems: "center", padding: 14, borderRadius: 16,
    borderWidth: 0.5, gap: 14,
  ...
  cardTitle: { fontSize: 15, fontWeight: "600", letterSpacing: -0.1, flexShrink: 1 },
  plusBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 5 },
  plusBadgeText: { color: "#ffffff", fontSize: 9, fontWeight: "700", letterSpacing: 0.5 },
  cardSubtitle: { fontSize: 12.5, marginTop: 2, lineHeight: 17 },
  metaRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 6 },
  metaItem: { flexDirection: "row", alignItems: "center", gap: 4 },
  metaDot: { width: 2, height: 2, borderRadius: 1 },
  cardMeta: { fontSize: 11.5 },
  chevWrap: { width: 28, height: 28, borderRadius: 8, alignItems: "center", justifyContent: "center" },
  footer: { paddingVertical: 10, borderTopWidth: 0.5, alignItems: "center" },
  footerInner: { flexDirection: "row", alignItems: "center", gap: 6 },
  footerText: { fontSize: 10.5, letterSpacing: 0.3 },
});
